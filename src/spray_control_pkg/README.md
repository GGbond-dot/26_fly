...
启用条码任务（`enable_barcode_task: True`）时，柱子改为**空中识别**：节点先正常起飞，起飞后立即发
`/pillar_detect_enable=true` 开一个时长 `pillar_detect_window_sec`（默认 4s）的检测窗，窗内 `pillar_detector_tf`
持续把 `/scan` 点变换到 **map 系**、按固定 ROI 攒帧（在 map 系框 ROI，无人机边飞边转都不影响指向柱子）；
关窗发 `/pillar_detect_enable=false` → 检测器聚类后把结果发到 `/detected_pillars`（按票数降序，取首个=最佳）。
节点收到后锁定坐标并在**返航(return)航点之前**插入"接近(巡航高)→观察(降到 `barcode_target_z_cm` 悬停读)→拉高(回巡航高)"
三段航点——柱子低于巡航高度，覆盖结束顺路降读再返航，条码数字仅圆周降落用，越晚读越省飞行。
相机朝向由 `barcode_cam_axis` 决定后撤方向与 yaw。检测窗内没攒够票（`/detected_pillars` 为空）则跳过条码任务，正常返航。
...
| 发布 | `/pillar_detect_enable` | Bool | 起飞后开/关柱子检测窗（true 攒帧 / false 聚类发布），驱动 `pillar_detector_tf` |
...
| 订阅 | `/detected_pillars` | Float32MultiArray | `[x_m, y_m, ...]` 多杆按票数降序，取首个=最佳（pillar_detector_tf，空中检测窗输出） |

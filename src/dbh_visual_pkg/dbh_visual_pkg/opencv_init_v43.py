...
DEFAULT_CAMERA_NAME = "DECXIN"


def classify_circle_size(area_ratio: float) -> tuple[str, float, float]:
    if area_ratio <= 0:
        return "未知", 0.0, 0.0

    frame_side_cm = 15.0
    classes = [("小", 5.0), ("中", 8.0), ("大", 11.0)]

    expected: list[tuple[str, float, float]] = []
    for name, d in classes:
        ratio = math.pi * (d * 0.5) ** 2 / (frame_side_cm**2)
        expected.append((name, d, ratio))

    best_name = "未知"
    best_d = 0.0
    best_ratio = 0.0
    best_diff = float("inf")
    for name, d, ratio in expected:
        diff = abs(area_ratio - ratio)
        if diff < best_diff:
            best_diff = diff
            best_name = name
            best_d = d
    ...
    rel_err = best_diff / max(1e-6, best_ratio)
    confidence = max(0.0, min(1.0, 1.0 - rel_err))
    return best_name, best_d, confidence


def size_name_to_id(size_name: str) -> int:
    if size_name == "小":
        return 1
    if size_name == "中":
        return 2
...
def init_ros2_publisher(topic: str) -> Optional[tuple[Any, Any, Any]]:
    try:
        ...
        node = rclpy.create_node("visual_result_publisher")
        pub = node.create_publisher(String, topic, 10)
        print(f"[信息] ROS2已启用，topic={topic}")
        return rclpy, node, pub
    except Exception as exc:
        print(f"[警告] ROS2初始化失败，已跳过发布：{exc}")
        return None


def shutdown_ros2(ros_ctx: Optional[tuple[Any, Any, Any]]) -> None:
    if ros_ctx is None:
        return
    rclpy, node, _ = ros_ctx
    try:
        node.destroy_node()
    except Exception:
        pass
...
def draw_overlay(
    ...
    h, w = frame.shape[:2]
    cx0, cy0 = w // 2, h // 2
    cv2.line(frame, (cx0 - 10, cy0), (cx0 + 10, cy0), (0, 255, 255), 1, cv2.LINE_AA)
    cv2.line(frame, (cx0, cy0 - 10), (cx0, cy0 + 10), (0, 255, 255), 1, cv2.LINE_AA)
    cv2.circle(frame, (cx0, cy0), 2, (0, 255, 255), -1)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Robust square+circle ratio detector for moving flight scenes.")
    parser.add_argument("--camera", type=int, default=0, help="Camera index.")
    parser.add_argument(
        "--camera-name",
        type=str,
        default=DEFAULT_CAMERA_NAME,
        help="Preferred camera name keyword. Default is DECXIN.",
    )
    parser.add_argument("--width", type=int, default=640, help="Capture width.")
    parser.add_argument("--height", type=int, default=480, help="Capture height.")
    parser.add_argument("--fps", type=int, default=60, help="Preferred camera FPS.")
    parser.add_argument("--min-area", type=float, default=900.0, help="Minimum contour area to keep.")
    parser.add_argument("--stable-frames", type=int, default=1, help="Frames needed before lock.")
    parser.add_argument("--max-missed", type=int, default=34, help="Allowed missed frames before reset.")
    parser.add_argument("--method", choices=["adaptive", "otsu"], default="adaptive", help="Threshold method.")
    parser.add_argument("--mirror", action="store_true", help="Mirror the camera frame.")
    parser.add_argument("--show-binary", action="store_true", help="Show binary debug window on startup.")
    parser.add_argument("--log-interval", type=int, default=10, help="日志间隔帧，0表示每个有效帧都打印")
    parser.add_argument("--size-confirm-frames", type=int, default=10, help="分类确认连续帧数")
    parser.add_argument("--ros2-disable", action="store_true", help="禁用ROS2发布")
    parser.add_argument("--ros2-topic", type=str, default="visual_result", help="ROS2发布话题名")
    parser.add_argument("--publish-interval-frames", type=int, default=5, help="ROS2发布帧间隔")
    return parser
    ...
    try:
        ...
        for idx, device_name in enumerate(devices):
            if name in device_name.lower():
                print(f"Camera name matched: index={idx}, device={device_name}")
                return idx
        print(f"Camera name '{preferred_name}' not found. Fallback to index {fallback_index}.")
    except Exception as exc:
        print(f"Camera-name resolve failed ({exc}). Fallback to index {fallback_index}.")

    return fallback_index


def _on_trackbar(_: int) -> None:
    pass


def create_tuning_panel(config: DetectorConfig, tracker: StableRectangleTracker) -> None:
    cv2.namedWindow("Tuning", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Tuning", 700, 520)

    cv2.createTrackbar("MinArea", "Tuning", int(config.min_area), 12000, _on_trackbar)
    cv2.createTrackbar("AdaptiveC(+10)", "Tuning", int(config.adaptive_c + 10), 40, _on_trackbar)
    cv2.createTrackbar("BlackVMax", "Tuning", int(config.black_v_max), 180, _on_trackbar)
    cv2.createTrackbar("BlackSMax", "Tuning", int(config.black_s_max), 255, _on_trackbar)
    cv2.createTrackbar("BlackKernel", "Tuning", int(config.black_kernel_size), 15, _on_trackbar)
    cv2.createTrackbar("AngleTol", "Tuning", int(config.angle_tolerance), 40, _on_trackbar)
    cv2.createTrackbar("LockFrames", "Tuning", int(tracker.required_hits), 20, _on_trackbar)
    cv2.createTrackbar("HoldFrames", "Tuning", int(tracker.max_missed), 90, _on_trackbar)

    cv2.createTrackbar("CircleMinArea", "Tuning", int(config.circle_min_area), 5000, _on_trackbar)
    cv2.createTrackbar("CircleCirc(%)", "Tuning", int(config.circle_circularity_min * 100), 100, _on_trackbar)
    cv2.createTrackbar("CircleSolid(%)", "Tuning", int(config.circle_solidity_min * 100), 100, _on_trackbar)
    cv2.createTrackbar("CircleAspect(x100)", "Tuning", int(config.circle_aspect_max * 100), 300, _on_trackbar)
    cv2.createTrackbar("CircleMargin", "Tuning", int(config.circle_roi_margin), 30, _on_trackbar)
    cv2.createTrackbar("CenterOffset(%)", "Tuning", int(config.circle_center_offset_max_ratio * 100), 80, _on_trackbar)

    cv2.createTrackbar("RatioOutlier(%)", "Tuning", int(config.ratio_outlier_percent * 100), 80, _on_trackbar)
    cv2.createTrackbar("RatioWindow", "Tuning", int(config.ratio_window_size), 20, _on_trackbar)
    cv2.createTrackbar("RatioConf(%)", "Tuning", int(config.ratio_confidence_min * 100), 100, _on_trackbar)


def sync_from_tuning_panel(config: DetectorConfig, tracker: StableRectangleTracker, ratio_filter: RobustRatioFilter) -> bool:
    changed_ratio_filter = False
    try:
        config.min_area = max(300.0, float(cv2.getTrackbarPos("MinArea", "Tuning")))
        config.adaptive_c = cv2.getTrackbarPos("AdaptiveC(+10)", "Tuning") - 10
        config.black_v_max = max(10, cv2.getTrackbarPos("BlackVMax", "Tuning"))
        config.black_s_max = max(10, cv2.getTrackbarPos("BlackSMax", "Tuning"))
        config.black_kernel_size = max(3, cv2.getTrackbarPos("BlackKernel", "Tuning"))
        config.angle_tolerance = float(max(5, cv2.getTrackbarPos("AngleTol", "Tuning")))
        tracker.required_hits = max(1, cv2.getTrackbarPos("LockFrames", "Tuning"))
        tracker.max_missed = max(1, cv2.getTrackbarPos("HoldFrames", "Tuning"))

        config.circle_min_area = float(max(30, cv2.getTrackbarPos("CircleMinArea", "Tuning")))
        config.circle_circularity_min = max(0.1, cv2.getTrackbarPos("CircleCirc(%)", "Tuning") / 100.0)
        config.circle_solidity_min = max(0.1, cv2.getTrackbarPos("CircleSolid(%)", "Tuning") / 100.0)
        config.circle_aspect_max = max(1.0, cv2.getTrackbarPos("CircleAspect(x100)", "Tuning") / 100.0)
        config.circle_roi_margin = max(0, cv2.getTrackbarPos("CircleMargin", "Tuning"))
        config.circle_center_offset_max_ratio = max(0.05, cv2.getTrackbarPos("CenterOffset(%)", "Tuning") / 100.0)

        old_outlier = config.ratio_outlier_percent
        old_window = config.ratio_window_size
        old_conf = config.ratio_confidence_min

        config.ratio_outlier_percent = max(0.01, cv2.getTrackbarPos("RatioOutlier(%)", "Tuning") / 100.0)
        config.ratio_window_size = max(3, cv2.getTrackbarPos("RatioWindow", "Tuning"))
        config.ratio_confidence_min = max(0.05, cv2.getTrackbarPos("RatioConf(%)", "Tuning") / 100.0)

        if (
            abs(old_outlier - config.ratio_outlier_percent) > 1e-6
            or old_window != config.ratio_window_size
            or abs(old_conf - config.ratio_confidence_min) > 1e-6
        ):
            ratio_filter.outlier_percent = config.ratio_outlier_percent
            ratio_filter.confidence_min = config.ratio_confidence_min
            if ratio_filter.window.maxlen != max(3, config.ratio_window_size):
                ratio_filter.window = deque(ratio_filter.window, maxlen=max(3, config.ratio_window_size))
            changed_ratio_filter = True

    except cv2.error:
        pass

    return changed_ratio_filter


def push_to_tuning_panel(config: DetectorConfig, tracker: StableRectangleTracker) -> None:
    try:
        cv2.setTrackbarPos("MinArea", "Tuning", int(max(300, min(12000, config.min_area))))
        cv2.setTrackbarPos("AdaptiveC(+10)", "Tuning", int(max(0, min(40, config.adaptive_c + 10))))
        cv2.setTrackbarPos("BlackVMax", "Tuning", int(max(10, min(180, config.black_v_max))))
        cv2.setTrackbarPos("BlackSMax", "Tuning", int(max(10, min(255, config.black_s_max))))
        cv2.setTrackbarPos("BlackKernel", "Tuning", int(max(3, min(15, config.black_kernel_size))))
        cv2.setTrackbarPos("AngleTol", "Tuning", int(max(5, min(40, config.angle_tolerance))))
        cv2.setTrackbarPos("LockFrames", "Tuning", int(max(1, min(20, tracker.required_hits))))
        cv2.setTrackbarPos("HoldFrames", "Tuning", int(max(1, min(90, tracker.max_missed))))

        cv2.setTrackbarPos("CircleMinArea", "Tuning", int(max(30, min(5000, config.circle_min_area))))
        cv2.setTrackbarPos("CircleCirc(%)", "Tuning", int(max(10, min(100, config.circle_circularity_min * 100))))
        cv2.setTrackbarPos("CircleSolid(%)", "Tuning", int(max(10, min(100, config.circle_solidity_min * 100))))
        cv2.setTrackbarPos("CircleAspect(x100)", "Tuning", int(max(100, min(300, config.circle_aspect_max * 100))))
        cv2.setTrackbarPos("CircleMargin", "Tuning", int(max(0, min(30, config.circle_roi_margin))))
        cv2.setTrackbarPos("CenterOffset(%)", "Tuning", int(max(5, min(80, config.circle_center_offset_max_ratio * 100))))

        cv2.setTrackbarPos("RatioOutlier(%)", "Tuning", int(max(1, min(80, config.ratio_outlier_percent * 100))))
        cv2.setTrackbarPos("RatioWindow", "Tuning", int(max(3, min(20, config.ratio_window_size))))
        cv2.setTrackbarPos("RatioConf(%)", "Tuning", int(max(5, min(100, config.ratio_confidence_min * 100))))
    except cv2.error:
        pass
...
def main() -> int:
    ...
    selected_index = resolve_camera_index(args.camera_name, args.camera)
    cap = open_camera(selected_index, args.width, args.height, args.fps)
    if not cap.isOpened():
        print("Failed to open camera. Check index, permissions, and whether another app is using it.")
        return 1

    tracker = StableRectangleTracker(
        required_hits=max(1, args.stable_frames),
        max_missed=max(1, args.max_missed),
        pred_decay=config.track_pred_decay,
        pred_max_step_ratio=config.track_pred_max_step_ratio,
        gate_base=config.track_gate_base,
        gate_miss_gain=config.track_gate_miss_gain,
        gate_speed_gain=config.track_gate_speed_gain,
        gate_speed_norm=config.track_gate_speed_norm,
        size_ratio_min=config.track_size_ratio_min,
        size_ratio_max=config.track_size_ratio_max,
        size_ratio_min_miss=config.track_size_ratio_min_miss,
        size_ratio_max_miss=config.track_size_ratio_max_miss,
        velocity_blend_old=config.track_velocity_blend_old,
        switch_required_hits=config.track_switch_required_hits,
        pending_iou_min=config.track_pending_iou_min,
    )
    show_binary = bool(args.show_binary)
    binary_window_open = False
    cam_fps = cap.get(cv2.CAP_PROP_FPS)
    last_tick = time.perf_counter()
    loop_fps = 0.0

    ratio_filter = RobustRatioFilter(
        alpha=config.circle_ratio_ema_alpha,
        outlier_percent=config.ratio_outlier_percent,
        window_size=config.ratio_window_size,
        confidence_min=config.ratio_confidence_min,
    )

    circle_last: Optional[dict[str, Any]] = None
    circle_misses = 0
    frame_idx = 0
    publish_counter = 0
    size_candidate = ""
    size_streak = 0
    size_confirmed = ""
    ros_ctx = init_ros2_publisher(args.ros2_topic) if not bool(args.ros2_disable) else None
    ros_fallback_terminal = (not bool(args.ros2_disable)) and (ros_ctx is None)
    if ros_fallback_terminal:
        print("[信息] ROS2启动失败，已回退为终端输出话题内容")

    print("Camera started.")
    print(f"Using camera index: {selected_index}")
    print("Controls: q/esc=quit, m=switch threshold, b=toggle binary, r=reset tracker")
    print("Trackbar: drag values in Tuning window, press t to reopen window")

    create_tuning_panel(config, tracker)
    push_to_tuning_panel(config, tracker)
    ...
    while True:
        ...
        if delta > 1e-6:
            inst_fps = 1.0 / delta
            loop_fps = inst_fps if loop_fps == 0.0 else (0.9 * loop_fps + 0.1 * inst_fps)
        ...
        ratio_filter_changed = sync_from_tuning_panel(config, tracker, ratio_filter)
        if ratio_filter_changed and circle_last is None:
            ratio_filter.reset()

        _, binary_primary = preprocess(frame, config)
        binary_black = build_black_mask(frame, config)
        binary_fused = fuse_binary_masks(binary_primary, binary_black, config)

        detection = detect_rectangle_with_fallback(binary_fused, binary_primary, config)
        state, confirmed = tracker.update(detection)

        circle_current: Optional[dict[str, Any]] = None
        allow_circle_hold = state.misses <= config.circle_detect_miss_tolerance and state.stable_hits >= max(2, tracker.required_hits)
        if state.polygon is not None and (confirmed or allow_circle_hold):
            circle_current = detect_circle_in_square(binary_fused, state.polygon, config)

        if circle_current is not None:
            smooth_ratio = ratio_filter.update(circle_current["ratio"], circle_current["confidence"])
            if smooth_ratio is not None:
                circle_current["ratio_smooth"] = smooth_ratio
                circle_last = circle_current
                circle_misses = 0
        else:
            circle_misses += 1
            if circle_misses > config.circle_hold_frames:
                circle_last = None
                ratio_filter.reset()

        circle_for_draw = circle_current if circle_current is not None else circle_last
        circle_hold = circle_current is None and circle_last is not None
        ...
        center_dy = 0.0
        center_err = 0.0
        if circle_for_draw is not None:
            ratio_raw = float(circle_for_draw.get("ratio", 0.0))
            ratio_ema = float(circle_for_draw.get("ratio_smooth", ratio_raw))
            size_name, size_d, size_conf = classify_circle_size(ratio_ema)

            if size_name == size_candidate:
                size_streak += 1
            else:
                size_candidate = size_name
                size_streak = 1

            if size_name != "未知" and size_streak >= max(1, int(args.size_confirm_frames)):
                size_confirmed = size_name

            cx, cy = circle_for_draw["center"]
            frame_h, frame_w = frame.shape[:2]
            center_dx = float(cx - (frame_w * 0.5))
            center_dy = float(cy - (frame_h * 0.5))
            center_err = float(math.hypot(center_dx, center_dy))
        else:
            size_candidate = ""
            size_streak = 0

        vis = frame.copy()
        draw_overlay(vis, state, confirmed, config.threshold_method, loop_fps, cam_fps, circle_for_draw, circle_hold)

        cv2.imshow("Camera - Robust Square/Circle Ratio Detection", vis)
        if show_binary:
            binary_debug = cv2.hconcat([binary_primary, binary_black, binary_fused])
            cv2.imshow("Binary Debug", binary_debug)
            binary_window_open = True
        else:
            if binary_window_open:
        ...
        if circle_for_draw is not None:
            cls_name = size_confirmed if size_confirmed else size_name
            cls_id = size_name_to_id(cls_name)
            if cls_id > 0:
                publish_counter += 1
                if publish_counter >= max(1, int(args.publish_interval_frames)):
                    publish_counter = 0
                    payload = f"{cls_id},{int(round(center_dx))},{int(round(center_dy))}"
                    if ros_ctx is not None:
                        rclpy, node, pub = ros_ctx
                        try:
                            from std_msgs.msg import String  # type: ignore
                            ...
                            # 终端输出与ROS话题内容完全同频、同数据（仅增加字段说明）
                            print(f"[发送] 类型={cls_id}，dx={int(round(center_dx))}，dy={int(round(center_dy))}")
                        except Exception:
                            pass
                    elif ros_fallback_terminal:
                        # ROS2启动失败时，终端输出本应发布到话题的数据。
                        print(f"[发送-回退] 类型={cls_id}，dx={int(round(center_dx))}，dy={int(round(center_dy))}")
        elif circle_for_draw is None:
            publish_counter = 0
        ...
        if key in (27, ord("q")):
            break
        if key == ord("m"):
            config.threshold_method = "otsu" if config.threshold_method == "adaptive" else "adaptive"
            print(f"Threshold method switched to: {config.threshold_method}")
        if key == ord("b"):
            show_binary = not show_binary
            print(f"Binary window: {'ON' if show_binary else 'OFF'}")
        if key == ord("r"):
            tracker.reset()
            circle_last = None
            circle_misses = 0
            ...
            size_confirmed = ""
            publish_counter = 0
            print("Tracker + circle state reset.")
        if key in (ord("+"), ord("=")):
            config.min_area += 300
            print(f"min_area -> {config.min_area:.0f}")
            push_to_tuning_panel(config, tracker)
        if key in (ord("-"), ord("_")):
            config.min_area = max(300.0, config.min_area - 300)
            print(f"min_area -> {config.min_area:.0f}")
            push_to_tuning_panel(config, tracker)
        if key == ord("]"):
            config.adaptive_c = min(30, config.adaptive_c + 1)
            print(f"adaptive_c -> {config.adaptive_c}")
            push_to_tuning_panel(config, tracker)
        if key == ord("["):
            config.adaptive_c = max(-10, config.adaptive_c - 1)
            print(f"adaptive_c -> {config.adaptive_c}")
            push_to_tuning_panel(config, tracker)
        if key == ord("t"):
            create_tuning_panel(config, tracker)
            push_to_tuning_panel(config, tracker)
            print("Tuning window reopened.")

    cap.release()
    cv2.destroyAllWindows()
    shutdown_ros2(ros_ctx)
    return 0

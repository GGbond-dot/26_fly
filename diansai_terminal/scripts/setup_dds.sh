#!/usr/bin/env bash
# D题 盘点无人机 ←→ 地面站 跨机 DDS / ROS2 通信环境配置（地面站端）
#
# 在启动地面站前 source 本脚本。地面站端与飞机端 **必须配置一致**
# （飞机端见 26fly 仓库 scripts/setup_dds.sh，两边 ROS_DOMAIN_ID 要相同）：
#
#   source /opt/ros/humble/setup.bash
#   source <dir>/scripts/setup_dds.sh
#   python3 -m ground_station.main
#
# 关键点（两端必须一致）：
#   ROS_DOMAIN_ID         同域才能互相发现，固定 26（强制覆盖外部值，
#                         临时改域用 DIANSAI_DOMAIN_ID）
#   ROS_LOCALHOST_ONLY=0  跨机通信必须为 0（=1 只走本机回环，发现不到飞机）
#   RMW_IMPLEMENTATION    统一用 FastDDS（rmw_fastrtps_cpp），两端 RMW 必须相同
#
# 注意：本脚本只导出环境变量，不 source ROS。
...
# 发现不到飞机时，指定实际联网的网卡（如无线 wlan0）把 FastDDS 限定到它：
